<template>
  <div class="md-layout">
    ...
    <h3 class="page_title">{{this.companyName}}</h3>
    <div class="md-layout-item md-medium-size-50 md-xsmall-size-100 md-size-25">
      <stats-card header-color="blue">
        <template slot="header">
          ...
          <h3 class="title">{{this.companyClicks}}</h3>
        </template>
...
export default {
  ...
  methods: {
    ...
    getCompany() {
      ...
      let companyName;
      let clicks;
      this.$bind('company', db.collection("building")
        .doc(this.$route.params.building_id)
        .collection("companies")
        .doc(this.$route.params.company_id))
        .then((data) => {
          this.companyName = data.name
          this.companyClicks = data.clicks
          this.pageSearched = data.searched_amount
        })
